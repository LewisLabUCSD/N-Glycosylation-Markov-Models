...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        hErrorbar(col) = errorbar(mean(x,1), values(xOrder,col), lowerErrors(xOrder,col), upperErrors(xOrder, col), 'or');
        set(hErrorbar(col), 'marker', 'none','Color',[0,0,0])
    end
else
    if verLessThan('matlab', '8.4')
        % Original graphics:
    ...
    hErrorbar = errorbar(mean(x,1), values, lowerErrors, upperErrors, 'or');
    set(hErrorbar, 'marker', 'none','Color',[0,0,0])
end
