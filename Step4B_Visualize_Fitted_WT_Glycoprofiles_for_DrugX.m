...
OptimizationResults = LoadOptimizationResults({'OptimizationResults_DrugXWT'});
...
% Progress bar
f= waitbar(0,['Compute models and render visualization for: ',strrep(ProfSel,'_','/'),sprintf(' (%d/%d)',1,length(ProfSel))]);
...
for a = 1:length(ProfSel)
    ...
    if a+1<length(ProfSel)
        waitbar(a/length(ProfSel),f, ['Compute models and render visualization for: ',ProfSel{a+1},sprintf('(%d/%d)',a,length(ProfSel))]);
    end
end
delete(f);

save('Data/ProcessedModels_DrugX.mat','OptimizationResults');